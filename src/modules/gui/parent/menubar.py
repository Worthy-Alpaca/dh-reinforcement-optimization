...
def Titlebar(
    ...
    title_bar_title = ttk.Label(
        title_bar, text=title_text, font=("Copperplate Gothic Bold", 20, "italic")
    )
    ...
    def resizex(event):
        ...
        if (
            root.winfo_width() <= min_width
            and difference > 0
            or root.winfo_width() > min_width
        ):
            try:
                root.geometry(
    ...
    def resizey(event):
        ...
        if (
            root.winfo_height() <= min_height
            and difference > 0
            or root.winfo_height() > min_height
        ):
            try:
                root.geometry(
